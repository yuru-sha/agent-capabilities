---
name: create-pr
description: Create a ready-for-review GitHub pull request from the current implementation branch after explicit user authorization, with scoped diff checks, verification, commit/push, and post-creation status review. Use create-draft-pr when the PR must remain a draft.
---

# Create a ready pull request

Use this skill when the user explicitly asks to create, open, or submit a
non-draft GitHub pull request. A PR request authorizes the normal
commit/push/PR workflow for the current scoped changes; it does not authorize
merging, force-pushing, deleting branches/worktrees, or changing unrelated
files. Use `create-draft-pr` when review should start later.
